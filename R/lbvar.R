...
#' @return An object with S3 class "lbvar".
...
#'
#' Garcia, Medeiros and Vasconcelos (2017).
#' @seealso \code{\link{predict}}, \code{\link{irf}}, \code{\link{fitLambda}}
...
lbvar=function (Y, p = 1, delta = 0, lambda = 0.05, xreg = NULL,ps=FALSE,tau=10*lambda)
{
  ...
  if (length(xreg) > 0) {
    ...
    Xreg = cbind(Xreg, utils::tail(xreg, nrow(Xreg)))
  }
  ...
  residuals=utils::tail(Y,nrow(fitted))-fitted
